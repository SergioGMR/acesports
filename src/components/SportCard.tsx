...
export default function SportCard({ event, timeZone }: { event: SportEvent, timeZone: string }) {
    dayjs.extend(utc);
    ...
    return (
        ...
        >
            <div
                className="z-0 absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-800 to-transparent opacity-90 rounded-xl"
            >
            </div>
            ...
            <div className="flex justify-between text-center">
                <div className="flex flex-col gap-y-2 w-1/3">
                    <span
                        className="bg-yellow-500 searchable text-black text-xs font-bold py-1 px-3 rounded-full capitalize"
                    >{event.sport}</span>
                    {
                ...
                <div className="flex flex-col gap-y-2">
                    <span
                        className="bg-blue-500 z-20 overflow-hidden truncate text-white text-xs font-bold py-1 px-3 rounded-full searchable capitalize"
                    >{event.date.day}</span>
                    <span
                        ...
                        <>
                            ...
                            {event.teams.local.image && event.teams.visitor.image && (
                                <section className="flex gap-x-3 items-center justify-center">
                                    ...
                                    <div className="text-lg font-semibold text-orange-500 my-4">V.S.</div>
                                    <div>
                                        <img
                                            className="h-auto w-auto"
            ...
            <div
                className="absolute inset-0 flex items-end justify-center z-10 bottom-4 gap-x-2"
            >
                {
                    event.links && event.links.map((link: string, index: number) => (
                        <a
                            href={link}
                            target="_blank"
